...
func (r *repository) SavePostgres(ctx context.Context, record database.ContentRecord) error {
	query := `
	INSERT INTO content_records (sha256_hash, creator_address, phash, timestamp, ipfs_cid, ai_tool)
	VALUES ($1, $2, $3, $4, $5, $6)
	ON CONFLICT (sha256_hash) DO NOTHING;`

	_, err := r.db.ExecContext(ctx, query, record.Sha256Hash, record.CreatorAddress, record.PHash, record.Timestamp, record.IpfsCid, record.AiTool)
	return err
}

func (r *repository) GetPostgres(ctx context.Context, hash string) (*database.ContentRecord, error) {
	query := `
	SELECT sha256_hash, creator_address, phash, timestamp, ipfs_cid, ai_tool
	FROM content_records
	WHERE sha256_hash = $1;`

	var record database.ContentRecord
	err := r.db.QueryRowContext(ctx, query, hash).Scan(&record.Sha256Hash, &record.CreatorAddress, &record.PHash, &record.Timestamp, &record.IpfsCid, &record.AiTool)
